...
-- Run with the service role (SQL editor). Every row in the output should read PASS.
...
begin
  ...
  insert into res values ('A reads B approved question', '0 rows', n || ' rows');

  select count(*) into n from public.institutes;
  insert into res values ('A sees only own institute', '1 rows', n || ' rows');

  insert into res values ('has_institute_role(A, B)', 'false',
    public.has_institute_role('11111111-1111-1111-1111-111111111111','bbbbbbbb-0000-0000-0000-000000000002','faculty')::text);
  insert into res values ('has_institute_role(A, A)', 'true',
    public.has_institute_role('11111111-1111-1111-1111-111111111111','aaaaaaaa-0000-0000-0000-000000000001','faculty')::text);
...
select test, expected, actual,
       case when actual = expected or (expected = 'ok' and actual = 'ok') then 'PASS' else 'FAIL' end as result
from res;
